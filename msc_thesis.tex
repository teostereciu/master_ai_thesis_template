...
\frontmatter
        \chapter*{Abstract}
                \begin{adjustwidth}{4cm}{4cm}
                        Tool-use benchmarks assume that tools already exist. This thesis examines the step before: whether large language models (LLMs) can synthesize functional API tool servers from documentation, and whether those servers enable effective agent task completion. We introduce an end-to-end benchmark covering 14 REST APIs across five domains, in which a synthesis agent reads raw developer documentation and produces a runnable Model Context Protocol (MCP) server; a separate evaluation agent then attempts 13--25 natural-language tasks against it; and a third model judges each outcome.
