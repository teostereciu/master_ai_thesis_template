...
To isolate model effects from API-specific variation, we evaluate seven synthesis models on a common three-API set (GitHub, Stripe, Zulip) selected to span low, medium, and high familiarity. Figure~\ref{fig:model_capability} summarises per-model PASS distributions across this common set.

Sonnet~4.6 achieves the highest median PASS across the three APIs. GPT-5.2 and GPT-5.4 perform comparably, with GPT-5.4 showing an anomalous result on Stripe in the docs condition (8\% PASS) attributable to synthesis-level failure: the generated server contains broken tool stubs that prevent the agent from executing any tasks, rather than agent reasoning failures. This distinction is detectable via SYNTH: the Stripe SYNTH score for that run is also low, confirming the bottleneck is at synthesis.

Synthesis reliability --- the fraction of runs producing a scoreable (non-NaN) result --- scales with model capability. Qwen3-Coder and GPT-5.4-mini produce frequent synthesis crashes on harder APIs (Stripe, GitHub), inflating apparent failure rates and making per-API comparisons unreliable for those models. Larger models (Sonnet~4.6, GPT-5.2, GPT-5.4) show near-complete reliability across all three APIs.

The nodocs advantage observed in RQ4 is strongest for larger models. Sonnet~4.6 and GPT-5.2 improve or hold steady on GitHub when documentation is removed, consistent with strong parametric priors overriding documentation noise. Qwen3-Coder, by contrast, shows higher adherence in the mutated condition ($\sim$60--100\%), indicating it relies more heavily on in-context documentation and is correspondingly more sensitive to documentation quality.

This model-level variation in adherence is a confound when interpreting the aggregate adherence results in Section~\ref{sec:rq3}: the near-zero aggregate adherence for GitHub reflects a mixture of high-capability models ignoring mutations (parametric dominance) and lower-capability models failing to parse the mutated documentation correctly. Cross-model disaggregation is therefore necessary for a clean interpretation of the RQ3 findings.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{media/fig9_model_capability.pdf}
  \caption{PASS distributions for six synthesis models on the common
    three-API evaluation set (GitHub, Stripe, Zulip). Models ordered
    by capability (left to right). Box shows interquartile range;
    whiskers extend to full range. Synthesis reliability (fraction of
    non-NaN runs) is annotated above each box.}
...
\begin{figure*}[htbp]
  ...
  \caption{Per-API, per-model PASS breakdown across all synthesis conditions
    ...
    corresponds to one API; colours distinguish synthesis models. Gaps indicate
    runs with synthesis failures (NaN PASS). The GPT-5.4 Stripe anomaly
    (8\% docs) is the leftmost bar in the Stripe group.}
