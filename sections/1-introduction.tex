...
Our main findings are: (i)~synthesis is feasible for most APIs under the best conditions (frontier-tier median PASS~83\%), and synthesized servers match or exceed human-authored reference implementations on GitHub and Zulip (in particular, the no-docs condition exceeds the GitHub community server 65\% baseline, while the docs condition scores comparably; the Zulip docs-condition server exceeds its 68\% baseline); (ii)~removing documentation does not degrade---and sometimes improves---PASS on high-frequency APIs, because model weights already encode correct implementations and documentation adds context noise; (iii)~mutation adherence is near-zero for well-known public APIs, confirming that parametric knowledge dominates documentation for those APIs; (iv)~SYNTH and PASS are strongly correlated ($r = 0.937$, $n = 209$ across all models), establishing the LLM verdict on synthesis quality as a reliable and cheap proxy for downstream agent performance. We additionally present a preliminary probing analysis connecting parametric dominance to early-to-mid transformer layers, and derive practical guidelines for robust synthesis system design.
