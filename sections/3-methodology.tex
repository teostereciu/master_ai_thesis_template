...
80\,GB GPUs (160\,GB total VRAM, CUDA~12.2). Where results are missing, this reflects synthesis failures rather than intentional subset selection. Model comparison results are presented in Section~\ref{sec:model_comparison}.

\begin{table}[ht]
\centering
\scriptsize
\caption{Synthesis models evaluated. \emph{Access}: API provider or local inference. \emph{Params}: total parameter count; n.d.\ = not disclosed by provider. For Qwen3, 3B act.\ denotes activated parameters per token (mixture-of-experts architecture). GPT-5.4-mini was evaluated on GitHub, Stripe, and Zulip only.}
\label{tab:models}
...
\midrule
GPT-5.2              & OpenAI GPT-5    & Azure API            & n.d.  & 400K   \\
GPT-5.4              & OpenAI GPT-5    & Azure API            & n.d.  & 1.05M  \\
GPT-5.4-mini         & OpenAI GPT-5    & Azure API            & n.d.  & 1.05M  \\
Sonnet~4.6           & Anthropic Claude & GCP API             & n.d.  & 1M     \\
Haiku~4.5            & Anthropic Claude & GCP API             & n.d.  & 200K   \\
Gemini~3.1~Pro       & Google Gemini   & GCP API              & n.d.  & 1M     \\
...
\noindent
Several additional models were evaluated as synthesis candidates but excluded
from the full 14-API primary comparison due to systematic failures. GPT-4.1 produced
