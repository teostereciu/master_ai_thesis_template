...
Each dataset includes a \texttt{TASK.md} file that provides the synthesis agent with all information needed to produce a functional server. The specification covers: the target implementation framework
...
GPT-5.2 serves as the primary baseline, run across all 14 APIs and all three
conditions. GPT-5.4 is run across all 14 APIs as a more recent baseline.
Sonnet~4.6 and Haiku~4.5 (both Anthropic Claude) are run across a
representative subset of APIs, offering a strong/efficient tradeoff within the
same model family. Gemini~3.1~Pro provides a third model family for
comparison. Qwen3-Coder-Next and its base variant Qwen3-Coder-Next-Base are
...
\centering
\footnotesize
\caption{Synthesis models evaluated. \emph{Access}: API provider or local inference.}
\label{tab:models}
\setlength{\tabcolsep}{3.5pt}
\begin{tabular}{llll}
\toprule
\textbf{Model} & \textbf{Family} & \textbf{Access} \\
\midrule
GPT-5.2              & OpenAI GPT-5    & Azure API     \\
GPT-5.4              & OpenAI GPT-5    & Azure API     \\
Sonnet~4.6           & Anthropic Claude & GCP API      \\
Haiku~4.5            & Anthropic Claude & GCP API      \\
Gemini~3.1~Pro       & Google Gemini   & GCP API       &\\
Qwen3-Coder-Next     & Qwen3           & vLLM (2$\times$H100) \\
Qwen3-Coder-Next-Base & Qwen3          & vLLM (2$\times$H100) \\
\bottomrule
\end{tabular}
\end{table}
...
The harness launches the synthesized server as a subprocess via stdio transport and waits up to 30 seconds for a successful \texttt{list\_tools()} response before marking the run as an \textsc{environment} failure. Each task is subject to a per-task wall-time cap of 5 minutes; tool calls that hang beyond this limit are terminated and scored as \textsc{environment}. Write tasks are isolated by the naming prefix convention and cleaned up after each run to prevent state leakage between tasks.
...
For example, given 10 tasks with 7~\textsc{pass}, 2~\textsc{tool\_coverage},
and 1~\textsc{agent\_reasoning}, PASS $= 7/9 = 0.78$; the
\textsc{agent\_reasoning} failure is excluded from both numerator and
denominator.
