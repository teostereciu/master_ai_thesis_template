...
APIs range from high-familiarity, high-complexity (GitHub, Stripe) to low-familiarity, low-complexity (Alphavantage). The eBay suite provides three difficulty tiers within a single platform: the Buy, Commerce, and Sell sub-APIs differ substantially in documentation volume and endpoint coverage despite sharing the same host and OAuth model. Jira and the eBay Sell API represent the most challenging synthesis targets, with large hierarchical documentation and high parameter cardinality.
...
We evaluate seven synthesis models spanning four model families and two
inference backends. Table~\ref{tab:models} summarises their key properties.
All models are run across all 14 APIs and all three conditions where synthesis
succeeds; where results are missing, this reflects synthesis failures rather
than intentional subset selection. GPT-5.2 serves as the primary baseline.
GPT-5.4 is run as a more recent OpenAI baseline. Sonnet~4.6 and Haiku~4.5
(both Anthropic Claude) offer a strong/efficient tradeoff within the same
model family. Gemini~3.1~Pro provides a third model family for comparison. Qwen3-Coder-Next and its base variant Qwen3-Coder-Next-Base are
served via a dedicated vLLM inference server running on 2$\times$NVIDIA H100
80\,GB GPUs (160\,GB total VRAM, CUDA~12.2), allowing comparison of
instruction-tuned versus base model synthesis behaviour. These span a range of training
corpora, post-training regimes, and context window sizes, allowing us to
assess whether synthesis quality is model-specific or reflects a more general
capability. Model comparison results are presented in Section~\ref{sec:rq1}.

\begin{table}[ht]
\centering
\scriptsize
\caption{Synthesis models evaluated. \emph{Access}: API provider or local inference. \emph{Params}: total parameter count; n.d.\ = not disclosed by provider. For Qwen3, 3B act.\ denotes activated parameters per token (mixture-of-experts architecture).}
\label{tab:models}
...
Qwen3-Coder-Next     & Qwen3           & vLLM (2$\times$H100) & 80B (3B act.) & 262.144    \\
Qwen3-Coder-Next-Base & Qwen3          & vLLM (2$\times$H100) & 80B (3B act.) & 262.144    \\
\bottomrule
\end{tabular}
...
from the primary comparison due to systematic failures. GPT-4.1 produced
...
simplest APIs. Gemini~2.5~Pro and Gemini~3.5~Flash both exhibited an
