...
The synthesis agent operates in an agentic loop with access to a set of file-system tools. The primary synthesis models (GPT-5.2, GPT-5.4, and Qwen3-Coder-Next via vLLM) are provided six tools: \texttt{read\_file}, \texttt{write\_file}, \texttt{edit\_file}, \texttt{glob}, \texttt{grep}, and \texttt{list\_directory}. The agent receives the task specification and, depending on the synthesis condition, a populated documentation directory. It iterates freely until it produces the required deliverables. Synthesis models are run at their provider defaults unless otherwise noted; sampling temperature is not explicitly fixed across all models. Each API--condition pair is synthesized in a single run; multiple runs are performed only for the variance study reported in Section~\ref{sec:rq1}. For APIs whose documentation corpora exceed a model's effective context window, the agent reads files selectively rather than receiving all documentation concatenated upfront; no corpus was artificially truncated.
