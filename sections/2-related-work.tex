...
The agent benchmarking landscape spans web navigation (WebArena;~\citeauthor{webarena2023},~\citeyear{webarena2023}), computer control (OSWorld;~\citeauthor{osworld2024},~\citeyear{osworld2024}), and general open-ended
assistance (GAIA;~\citeauthor{gaia2023},~\citeyear{gaia2023}). These benchmarks treat the agent's capability to act as the object of study and abstract over the tool infrastructure. A related line of work examines agents that generate and execute code to solve tasks~\citep{codeact2024}, but the generated code is typically a
...
In-context learning (ICL), the ability of LLMs to adapt to new inputs from examples or instructions in the prompt, was systematically characterised by \cite{brown2020gpt3}. A common assumption in the software engineering community is that supplying live API documentation in the context window grounds the model's output, ensuring that generated code reflects the current API specification rather than stale parametric memory. Subsequent work has expanded on this assumption in several ways.
