...
MCP-Bench~\citep{mcpbench2025} scales this to 250 tools across twenty APIs
and twenty evaluated models; and MCPAgentBench~\citep{mcpagentbench2024}
...
Concurrent work has begun to close this gap. ToolFactory~\citep{toolfactory2025} proposes a pipeline that parses heterogeneous, unstructured REST API documentation and generates callable tool stubs --- explicitly targeting the
...
the middle. For tool synthesis over long documentation files, this implies that endpoints described far from the beginning or end of a document may be systematically under-represented in the
...
common entities are retrieved from weights even when context is available.
...
default --- a resistance we observe directly in our mutation experiments.
