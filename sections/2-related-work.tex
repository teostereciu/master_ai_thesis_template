...
This chapter reviews and positions the literature across five domains that together ground our methodology and findings. We begin with work on \textit{tool use and tool synthesis} (Section~\ref{sec:related_work:tools}), surveying benchmarks that evaluate agents against existing tool libraries and the small body of work that instead generates tools from documentation --- the setting closest to ours. We then review \textit{code generation} (Section~\ref{sec:related_work:code_gen}), focusing on evaluation regimes that go beyond static correctness to live API execution, where the failure modes we observe become visible. The next two sections address the core empirical phenomenon this thesis characterises: \textit{in-context learning and context faithfulness} (Section~\ref{sec:related_work:context}) examines when and how models follow instructions that conflict with prior knowledge, while \textit{parametric knowledge and knowledge conflicts} (Section~\ref{sec:related_work:param_conflict}) surveys work on how training-time representations resist contextual override. \textit{Mechanistic interpretability} (Section~\ref{sec:related_work:mechanistic}) provides the theoretical and methodological basis for our logit-lens probing analysis. Finally, we introduce the \textit{Model Context Protocol} (Section~\ref{sec:related_work:mcp}), the open standard our synthesis target must comply with and against which prior agent benchmarks are evaluated. Table~\ref{tab:related} summarises our positioning relative to prior work. 
...
\centering
\scriptsize
\caption{Positioning relative to prior work. \emph{Tool synthesis}: executable tool creation from documentation. \emph{Doc faithfulness}: output grounded in provided documentation. \emph{Mechanistic analysis}: internal representation probing.}
\label{tab:related}
